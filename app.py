...
<style>
    ...
    * { font-family: 'Poppins', sans-serif; }
    ...
    .stButton > button:hover {
        background: linear-gradient(135deg, #ffc8d0, #ff9ab8);
        color: white;
    }
    .chart-container {
        border: 1.5px solid #ffb6c1;
    ...
    .stProgress > div > div {
        background: linear-gradient(90deg, #ffb6c1, #ff8fab) !important;
    }
...
with s1:
    st.markdown("""<div class='stat-item'>
        <div class='stat-number'>94.07%</div>
...
with s4:
    st.markdown("""<div class='stat-item'>
        ...
        <div class='stat-label'>Hand Landmarks</div>
    </div>""", unsafe_allow_html=True)
...
with col1:
    ...
    class VideoProcessor(VideoProcessorBase):
        ...
        def recv(self, frame):
            ...
            if results.multi_hand_landmarks:
                ...
                self.confidence = round(max_proba * 100, 1)
    ...
    if ctx.video_processor:
        ...
        if st.session_state.history:
            history_html = "".join([f"<span class='history-letter'>{l}</span>"
                                   for l in st.session_state.history])
            st.markdown(f"""
            <div class='card' style='margin-top:8px;'>
                <div class='label'>Recent Letters</div>
                <div style='text-align:center; margin-top:8px;'>{history_html}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        <div class='about-box'>
            <div class='about-item'>
                <span>Algorithm</span>
                <span class='about-value'>Random Forest Classifier</span>
            </div>
            <div class='about-item'>
                <span>Framework</span>
                <span class='about-value'>MediaPipe + OpenCV</span>
            </div>
            <div class='about-item'>
                <span>Dataset</span>
                <span class='about-value'>ASL Alphabet — Kaggle</span>
            </div>
            <div class='about-item' style='border:none;'>
                <span>Hand Landmarks</span>
                <span class='about-value'>21 Keypoints per hand</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
